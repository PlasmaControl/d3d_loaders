...
class signal_ece(signal_2d):
    ...
    def _cache_data(self):
        ...
        prof_data = torch.tensor(np.stack([fp['ece'][f"tecef{channel:02d}"] for channel in self.channels],
                                        axis=1)[t_inds,:])
        fp.close()
        elapsed = time.time() - t0_p
...
class signal_uci_label_relaxed(signal_uci_label):
    """ Bill's AE labels. These are approximate as he only labeled a single time, 
    so we take assume AE mode happens in relaxed window +-500ms around labeled time. 
    These are used for testing and validation while the stricter +-250ms window
    is used for training only. 
    ...
    def __init__(self, shotnr, t_params, 
                 datapath="/projects/EKOLEMEN/aza_lenny_data1", device="cpu"):
        self.name = 'Relaxed UCI approximate AE labels'
        super().__init__(shotnr, t_params, datapath, device)
